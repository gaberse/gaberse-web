...
export default function HomeExplorer({
  ...
  function leave() {
    if (
      !touchMode &&
      scopeRef.current &&
      shouldResetExplorerOnPointerExit(scopeRef.current, document.activeElement)
    ) {
      dispatch({ type: "reset" });
    }
  }
  ...
  function onBlurCapture(event: FocusEvent<HTMLElement>) {
    if (
      scopeRef.current &&
      shouldResetExplorerOnFocusExit(scopeRef.current, event.relatedTarget)
    ) {
      dispatch({ type: "reset" });
    }
  }
  ...
  function onTouchEnd(event: TouchEvent<HTMLElement>) {
    ...
    if (start === null || end === undefined || Math.abs(start - end) < 36)
      return;
    ...
    return (
      <div className="explorer__artifact-drift">
        ...
        {card.media ? (
          ...
          <div className="explorer__placeholder" aria-hidden="true">
            {labels.placeholder && <span>{labels.placeholder}</span>}
          </div>
        )}
    ...
    >
      {showInterests && (
        <div className="explorer__intro">
          <p id={labelId} className="explorer__label">
            {labels.heading}
          </p>
            ...
            {cards.map((card, index) => {
              ...
              return card.href ? (
                ...
                <button
                  ...
                  onMouseEnter={hover(card.id)}
                  onFocus={() => preview(card.id)}
                  onClick={() => dispatch({ type: "activate", id: card.id })}
                  aria-pressed={committedInterest === card.id}
                  aria-controls={`${labelId}-${card.id}`}
                >
                  {label}
                </button>
              );
